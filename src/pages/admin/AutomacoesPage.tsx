 ...
 *
 * **Requisitos:**
 * * Auditoria da lógica de limpeza de strings para evitar duplicação de "- Largura Máxima".
 * * Validação de tipos de dados (float vs string) na carga útil enviada à Edge Function.
 * * Verificação de limites de caracteres para descrições reduzidas no ERP.
 *
 * **Passos:**
 * 1. **Coleta de Logs:** Verificar erros de "acao inválida" ou falhas de rede no console.
 * 2. **Análise de Regex:** Garantir que `LARG_MAX_RE` e `LMAX_ABREV_RE` removam instâncias globais.
 * 3. **Teste de Fluxo:** Realizar preview para validar a máscara visual antes da efetivação.
 */
