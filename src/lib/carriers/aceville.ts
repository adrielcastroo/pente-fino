...
const API_URL = import.meta.env.VITE_ACEVILLE_API_URL as string | undefined;
const TOKEN = import.meta.env.VITE_ACEVILLE_TOKEN as string | undefined;
...
export const aceville: CarrierAdapter = {
  ...
  async track(code): Promise<TrackResponse> {
    if (!API_URL || !TOKEN) throw new Error('Aceville não configurado (VITE_ACEVILLE_API_URL/TOKEN)');
    ...
    if (!res.ok) throw new Error(`Aceville HTTP ${res.status}`);
