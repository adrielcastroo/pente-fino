...
const API_URL = import.meta.env.VITE_SAOMIGUEL_API_URL as string | undefined;
const TOKEN = import.meta.env.VITE_SAOMIGUEL_TOKEN as string | undefined;
...
export const saoMiguel: CarrierAdapter = {
  ...
  async track(code): Promise<TrackResponse> {
    if (!API_URL || !TOKEN) throw new Error('São Miguel não configurado (VITE_SAOMIGUEL_API_URL/TOKEN)');
