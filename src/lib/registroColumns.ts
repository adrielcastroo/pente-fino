...
export type RegistroColumnKey = 'item' | 'nf' | 'm2' | 'largura' | 'mLinear' | 'lote' | 'endereco' | 'loteSistema';
...
const LAYOUTS = {
  ...
  diversos: ['item', 'nf', 'mLinear', 'lote', 'endereco', 'loteSistema'],
  pvt: ['nf', 'mLinear', 'lote'],
...
export function getRegistroColumns(rows: Registro[], fallbackMode: RegistroMode = 'manual'): RegistroColumn[] {
  ...
  if (modes.length === 1) {
    ...
    if (mode === 'diversos') {
      ...
      if (tipos.length === 1) {
        return (tipos[0] === 'pvt' ? LAYOUTS.pvt : LAYOUTS.diversos).map(key => COLUMN_MAP[key]);
      }
    }
  ...
  const mixedOrder: RegistroColumnKey[] = ['item', 'nf', 'm2', 'largura', 'mLinear', 'lote', 'endereco', 'loteSistema'];
