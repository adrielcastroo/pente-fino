...
/**
 * Card de KPI compartilhado entre todos os módulos.
 */
export function StatCard({
  ...
  return (
    <Card
      className={cn(
        'rounded-md border-2 backdrop-blur-xl transition-all duration-300 relative overflow-hidden group',
        v.border,
        v.bg,
        className,
      )}
    >
      ...
      <CardContent className="p-4 sm:p-6 tablet-portrait:p-2 text-center space-y-1 sm:space-y-2 tablet-portrait:space-y-0.5 relative z-10">
        {Icon && <Icon className={cn('w-4 h-4 mx-auto opacity-70', v.color)} />}
        <div
          className={cn(
