    ...
    >
      <div className="flex border-b-4 border-black flex-[1.1]">
        <div className="flex-1 p-2 flex flex-col justify-start overflow-hidden border-r-4 border-black">
          ...
          {has("descricao") && (
            <div className="mt-1 leading-tight line-clamp-2 overflow-hidden" style={{ fontSize: `${fs * 3.5}px` }}>
              {descLines.map((l, i) => (
                <div key={i}>{l}</div>
              ))}
            </div>
      ...
      {has("nfe") && (
        <div className="border-b-4 border-black px-2 py-1 flex flex-col justify-center gap-0.5 flex-[0.9] overflow-hidden">
          <span
            className="bg-black text-white font-bold px-1 py-0 w-fit leading-none"
            style={{ fontSize: `${fs * 1.8}px` }}
          >
            LOTE
          </span>
          <span
            className="font-black tracking-wide truncate leading-none"
            style={{ fontSize: `${fs * 4.5}px` }}
          >
            {data.lote}
          </span>
        </div>
