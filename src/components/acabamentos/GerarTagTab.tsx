...
export default function GerarTagTab({ onVerHistorico }: GerarTagTabProps = {}) {
  ...
  // ATENÇÃO: setLinhas NÃO deve ser chamado durante o render.
      ...
      if (recs.length > 0) {
        setLinhas((prev) => {
          ...
          for (const rec of recs) {
            ...
            // em vez de sobrescrever com a recomendacao automatica.
          ...
          const dedupado = dedupLinhasPorCode(resultado);
          ...
          if (dedupado.length === prev.length) {
            const iguais = dedupado.every((l, i) => {
              const p = prev[i];
