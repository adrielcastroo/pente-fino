...
import { Search, Loader2, Sparkles, Copy, Wand2 } from 'lucide-react';
...
export default function GerarTagTab() {
  ...
  const gerar = (input: string) => {
    const tag = normalizeTagFormatC(input);
    setTagGerada(tag);
  };
        ...
        <Card className="p-4 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Ex: Rollo Abs2.0 M Complemento_Day_Night T50 Motor CM-09/10"
          ...
          <div className="rounded border border-primary/40 bg-primary/5 p-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-[10px] uppercase text-muted-foreground flex items-center gap-1">
                <Sparkles className="h-3 w-3" /> TAG gerada (formato C · normalizada)
              </div>
            ...
            {selecionado?.ds_tag_calculada && (
              <div className="text-[10px] text-muted-foreground">
                TAG atual no Auge: <span className="font-mono">{selecionado.ds_tag_calculada}</span>
              </div>
            )}
          </div>
