      ...
      <div className="px-3 sm:px-4 py-2 surface-bg border-b border-border flex flex-wrap items-center gap-2 flex-shrink-0">
        <div className="flex items-center gap-2 surface-2-bg border border-border rounded-lg px-3 flex-1 min-w-0">
          ...
          <input
            ...
            className="border-none bg-transparent outline-none text-sm py-2 w-full"
            placeholder="Filtrar…"
            autoComplete="off"
          />
        </div>
        <div className="flex gap-1.5 items-center">
          <select
            ...
            className="border border-border rounded-lg px-2.5 py-2 text-xs text-muted-foreground bg-surface outline-none cursor-pointer"
          >
          ...
          <button onClick={exportExcel} className="p-2 rounded-lg border border-border hover:bg-surface-2 transition-colors" title="Exportar Excel">
            <Download className="w-4 h-4 text-muted-foreground" />
          </button>
          {registros.length > 0 && (
            <button onClick={handleClearAll} className="p-2 rounded-lg border border-border hover:bg-surface-2 transition-colors" title="Limpar tudo">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <th className="sticky top-0 z-10 surface-bg border-b-2 border-border px-3 py-2.5 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Lote</th>
                <th className="sticky top-0 z-10 surface-bg border-b-2 border-border px-3 py-2.5 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Cor</th>
                <th className="sticky top-0 z-10 surface-bg border-b-2 border-border px-3 py-2.5 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[80px]"></th>
              </tr>
            </thead>
            <tbody>
              <AnimatePresence initial={false}>
                {rows.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-sm font-mono">{r.largura.toFixed(2)}</td>
                    <td className="px-3 py-2.5 text-sm"><span className="badge-teal">{r.m2.toFixed(3)}</span></td>
                    <td className="px-3 py-2.5 text-sm font-mono max-w-[190px] overflow-hidden text-ellipsis whitespace-nowrap">
                      <span className="cursor-pointer text-primary hover:underline" onClick={() => copyText(r.lote)}>{r.lote}</span>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-muted-foreground max-w-[120px] overflow-hidden text-ellipsis whitespace-nowrap">{r.obs || ''}</td>
                    <td className="px-3 py-2.5">
                      <div className="row-acts">
                        <button onClick={() => copyText(r.lote)} className="p-1.5 rounded-md hover:bg-surface-2 transition-colors" title="Copiar lote">
                          <Copy className="w-3.5 h-3.5 text-muted-foreground" />
                        </button>
                        <button onClick={() => deleteRegistro(r.id)} className="p-1.5 rounded-md hover:bg-surface-2 transition-colors" title="Remover">
            ...
            {rows.length > 0 && (
              <tfoot>
                <tr className="navy-bg text-primary-foreground font-semibold font-mono text-xs sticky bottom-0">
                  <td colSpan={3} className="px-3 py-2.5">TOTAL — {rows.length} rolo{rows.length !== 1 ? 's' : ''}</td>
                  <td className="px-3 py-2.5">{formatML(totalML)}</td>
                  <td className="px-3 py-2.5">—</td>
                  <td className="px-3 py-2.5">{totalM2.toFixed(3)}</td>
